...
- `FUNC`はトップレベルのみ(ネスト禁止)。`STTYPE`・`SEL`も同様にネスト不可。`CLOS`だけは例外で`CLOS`の中に`CLOS`をネストできる(クロージャーを返すクロージャーの表現のため)
- 配列は1次元固定長のみ
...
- **`FUNC`/`STTYPE`/`SEL`はネスト禁止。ブロック抽出時に深さカウンタは不要。** `CLOS`だけは例外で`CLOS`の中に`CLOS`をネストできる(クロージャーを返すクロージャーを表現するため)。そのため`CLOS`の抽出だけはネスト深さ(`closureLevel`)を`parseBody`の再帰呼び出しに引き継ぐ必要がある(`&N`/`&L-N`の解決に使う)
...
上記の実装は`amivm_spec.md`の内容を実装済み(全命令・`:`区切り構文・`CASESEND`/`CASERECV`・`TYPE`系宣言・`CLOS`・CLIの`-o`/`-v`/`-i`など)。`test_ir/`配下に命令カテゴリ別の動作確認済みサンプルがある。新しい命令や構文上の変更を加えるときは、対応するテストIRを追加・更新し、実際に`amivm_spec.md`を更新した上でその内容が実装と一致していることを確認すること。
