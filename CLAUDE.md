...
`CLOS`の代入先は`local`(`%xxx`のみ)ではなく`shallow`という別カテゴリで、`$N`/`%xxx`/`@xxx`(関数レベル以上の変数)を許容する。`&N`(クロージャー引数)は`func`リテラルごとに閉じたスコープしか持たないため対象外。パラメータや`GVAR`で宣言済みのパッケージレベル変数にもクロージャーを代入できるということ(`test_ir/15_closure.ir`に`$1`・`@globalAdder`への代入例がある)。
