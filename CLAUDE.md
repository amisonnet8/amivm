...
- `FUNC`はトップレベルのみ(ネスト禁止)。`STTYPE`も同様にネスト不可。`IF`・`LOOP`・`CLOS`・`SEL`はいずれもネストできる(互いの本体の中に任意の組み合わせ・任意の深さで書ける)
- 配列は1次元固定長のみ
...
関数内変数・パラメータの実名は`<関数名>_amivm_function_<変数名>` / `<関数名>_amivm_function_paramN`という規則に従う。`main`も例外なくこの規則に従う(関数宣言自体は`main`のまま)。クロージャー引数(`&N`/`&L-N`)だけは例外で、関数名による修飾を持たない`amivm_closureL_paramN`になる(`L`はその`CLOS`のネスト深さ、`FUNC`直下を1として数える)。`CLOS`はネストできるため(後述)、内側と外側の`CLOS`が同じパラメータ番号を使うとGoの通常のスコープルールで外側がシャドーイングされてしまう。これを避けるためパラメータ名に階層`L`を埋め込んでいる。`&N`(階層省略形)は「自分がいる`CLOS`階層のN番目」を指し、外側の階層を参照したい場合は`&L-N`で階層を明示する。`CLOS`本体内で`VAR`宣言した変数は、`CLOS`自体が無名で専用の名前空間を持たないため、外側の`FUNC`の名前で修飾される(ネストしていても常に一番外側の`FUNC`の名前)。
...
**`CASE RECV`的な受信処理で`:=`を使うと変数シャドーイングが起きる。** `SEL`内の受信、`CHRECV`、その他あらゆる代入は、対象の変数が既に`VAR`/`GVAR`で宣言済みという前提のため、**常に`=`を使うこと。`:=`は使わない**。過去に`:=`を使ってしまい、外側で宣言した変数とは別の変数がブロックスコープ内に新規作成され、値が反映されない上に「未使用変数」の誤検出も引き起こすバグがあった。**唯一の例外が`METHOD`**(`local := variable.method`)で、これは対象変数が事前に`VAR`宣言されていない(=シャドーイングの土台となる外側の宣言がそもそも存在しない)ことを`local`カテゴリ(`%xxx`のみ)で構造的に保証した上での、意図的な`:=`使用。新しい命令を追加するときにこの例外を安易に広げないこと(メソッド呼び出しのパターン節参照)。
...
- **`FUNC`/`STTYPE`はネスト禁止。`IF`/`LOOP`/`CLOS`/`SEL`はいずれもネストできる。** `CLOS`の抽出は`&N`/`&L-N`解決のためネスト深さ(`closureLevel`)を`parseBody`の再帰呼び出しに引き継ぐ必要がある(前述)。`IF`/`LOOP`/`SEL`のネストは既存の`parseBody`(再帰下降)の枠組みでそのまま表現できる。`IF`はブロック開始行(`SEL`/`CLOS`と同様)として`parseBody`に認識させ、`ELIF`/`ELSE`/`ENDIF`を(ネストした内側の`IF`のものと混同しないよう)自分自身の再帰呼び出しの範囲内でだけ区切りとして認識する専用の走査を書く。`LOOP`も同様に`BREAK`/`CONTINUE`/`ENDLOOP`を伴うブロックとして扱う
...
- **`METHOD`だけは`=`ではなく`:=`を生成する。** `FNTYPE`で宣言した関数型がGoの実際のメソッド値の型と厳密に一致しないケースがあるための意図的な例外(前述「過去に発生した実装バグ」参照)。未使用変数の救済(`declaresVarDirect`)も`ast.DeclStmt`(`VAR`)だけでなく`ast.AssignStmt`かつ`Tok == token.DEFINE`(`METHOD`)を見るよう対応が必要
...
上記の`FNTYPE`+`FGET`パターンには弱点がある。`FGET`は`=`(既存変数への代入)なので、代入先の`%closeFn`をあらかじめ`VAR %closeFn ^CloseFn`で宣言しておく必要があり、`^CloseFn`(`FNTYPE`宣言)がGoの実際のメソッド値の型と寸分たがわず一致していないと`go/types`が代入エラーを返す。概念上は同じ型のつもりでも、Goの型同一性は名前まで含めて厳密なため、この不一致が実際に起きた。対処として`METHOD local variable method`(`local := variable.method`)を追加した。`FNTYPE`による事前の型宣言を要求せず、Goの`:=`にメソッド値の型をそのまま推論させることで、この不一致を構造的に起こらなくしている。`local`は`VAR`で事前宣言してはならない(`:=`は左辺の少なくとも1つが新規変数であることを要求するため)。`FNTYPE`の型が実際の型と一致すると分かっている場合は`FGET`、型の一致を気にせずGoに任せたい場合は`METHOD`、という使い分けになる。`test_ir/19_method.ir`に実例がある。
...
上記の実装は`amivm_spec.md`の内容を実装済み(全命令・`:`区切り構文・`CASESEND`/`CASERECV`・`TYPE`系宣言・`CLOS`・`IF`/`ELIF`/`ELSE`/`ENDIF`・`LOOP`/`BREAK`/`CONTINUE`/`ENDLOOP`・`ASSERT`・`METHOD`・CLIの`-o`/`-v`/`-i`など)。`test_ir/`配下に命令カテゴリ別の動作確認済みサンプルがある。新しい命令や構文上の変更を加えるときは、対応するテストIRを追加・更新し、実際に`amivm_spec.md`を更新した上でその内容が実装と一致していることを確認すること。
