...
上表の`0`,`1234`は10進の代表例であり、`imm`/`whole`/`integer`/`ordered`/`value`等いずれも16進(`0x`/`0X`)・8進(`0o`/`0O`)・2進(`0b`/`0B`)・桁区切りの`_`を含む整数リテラルの全形式を等しく許容する(詳細・拒否例は19節「整数リテラルに16進・8進・2進・桁区切り`_`を追加」参照)。
...
### 整数リテラルに16進・8進・2進・桁区切り`_`を追加

整数リテラルは元々10進(`0`単体、または`1`-`9`に`0`-`9`が続く形)と、その符号付き版(`-`を前置したもの)しか受け付けていなかった。ビットマスク・フラグ値のような用途では16進表記の方が読みやすく、Go自身が`0x`/`0o`/`0b`という基数プレフィックスと桁区切りの`_`(Go 1.13以降)をネイティブにサポートしているため、AMIVM側のトークン分類(`token.go`の`rePosInt`/`reNegInt`)もこれに合わせて拡張した。

生成されるGoコードにはトークンをそのまま出力するだけ(`atomToExpr`の`KZero`/`KPosInt`/`KNegInt`分岐は元から`ast.BasicLit{Kind: token.INT, Value: a.Raw}`というraw文字列そのままの組み立てだった)なので、`token.go`側の正規表現を広げる以外に変更は不要だった。旧式の先頭`0`だけによる8進表記(例: `0755`)は、`0xxx`/`0ooo`/`0bbb`との視覚的な区別が付きにくいため意図的に対象外とし、`0o`プレフィックスのみをサポートする。桁区切りの`_`は基数プレフィックスの直後、または数字と数字の間にのみ許容し、先頭・末尾・連続する`_`は不正な形式として弾く(Goの構文をそのまま踏襲)。

`imm`/`whole`/`integer`のように整数リテラルを許容する既存カテゴリは、いずれも`Kind`(`KZero`/`KPosInt`/`KNegInt`)単位で許可・拒否を判定しているため、`Category`側(`category.go`)の変更は不要だった。16進・8進・2進リテラルは(値がたまたま0であっても)全て`KPosInt`に分類され、`KZero`は10進の`0`単体のみを表す(この2つのKindは元々値の正負ではなくトークンの字面上の形を区別するためのものであり、命名は簡略化のための近似にすぎない)。
